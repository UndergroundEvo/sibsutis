...
#include <stack>
using namespace std;
static int i = 0, j = 0, i_point = 0, i_end = 0, j_point = 0, j_end = 0;//статические элементы (один вызываются за всю программу и дальше на них не работает присваивание)
class list {
protected:
    int num;
    int** matrix;
public:
    //list(int N) : matrix{0}{}
    list() : matrix{get_rand_matrix()} {} //инициализация элементов и отсутствующая перезагрузка конструкторов 0_0
    //list(int N) :matrix{get_matrix(N)} {}
    ~list(){}
    //метод генерации матрицы с клавы
    ...
    //метод генерации матрицы рандомом
    int** get_rand_matrix() {
        int N = 100;
        int** matrix = new int* [N];
        for (int i = 0;i < N;i++) {
...
protected:
    stack <int> almostmatrix;//класс в котором обьявляется стек и все d(-_^)
public:
    ~steck(){}
};
class queue :public steck { //класс в котором происходят все сортировки
protected:
public:
    ~queue(){}
    //сортировки в том же порядке что и int menu
...
int main() {
    ...
    int N = matrix_size();
    //list(N);
    queue almostmatrix;//элемент с которым идет вся работа, обьявлен с самого последнего класс в которого все унаследовано
